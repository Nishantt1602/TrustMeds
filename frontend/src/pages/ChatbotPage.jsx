...
const ChatbotPage = () => {
  const [messages, setMessages] = useState([
    { role: 'ai', text: "Hello! I'm your MedMarket AI Assistant. Tell me your symptoms or ask me about any medicine." }
  ]);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 h-[calc(100vh-80px)] flex flex-col">
      <div className="bg-teal-50 border border-teal-200 rounded-t-xl p-4 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">MedMarket AI Assistant</h1>
      ...
      <div className="flex-1 bg-white border-x border-gray-200 p-6 overflow-y-auto flex flex-col gap-6">
        {messages.map((msg, index) => (
          <div key={index} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-blue-100 text-blue-600' : 'bg-teal-100 text-teal-600'}`}>
              {msg.role === 'user' ? <User className="w-6 h-6" /> : <Bot className="w-6 h-6" />}
            </div>
            <div className={`p-4 rounded-2xl max-w-[80%] whitespace-pre-wrap ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-gray-100 text-gray-800 rounded-tl-none'}`}>
              {msg.text}
            </div>
          </div>
        ))}
